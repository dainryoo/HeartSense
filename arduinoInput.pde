...
boolean setupPort() {
  try {
    ...
    String portName = Serial.list()[2]; // Port usually seems to be 0, but maybe try 1 or 2 or etc. if it doesn't work?
...
void serialEvent(Serial port) {
  ...
  try {
    ...
    } else if (split(data, ",").length>1)
    {
      ...
      if (!plotLater)
      {
        if (tempBPM==0 && currBPM==0)
          currBPM = 90;
      }
      if (tempBPM>30 && currBPM<180)
          currBPM = tempBPM;
