...
int minGSR=2200, maxGSR=0;
float minRPM=60, maxRPM=0;
...
void TimeSync()
{
  ...
  while (ringCounter<rings.size()-1)
  {
    ...
    if (correctedTime.get(xethruCounter)>=(rings.get(ringCounter).percentage*video.duration()))
    {
  ...
  {
    if(rings.get(i).gsr>maxGSR)
      maxGSR = rings.get(i).gsr;
    if(rings.get(i).gsr<minGSR)
      minGSR = rings.get(i).gsr;
      
    if(rings.get(i).rpm>maxRPM)
      maxRPM = rings.get(i).rpm;
    if(rings.get(i).gsr<minRPM)
      minRPM = rings.get(i).rpm;
      
    println("Time: "+(rings.get(i).percentage*video.duration())+"    - RPM: " + rings.get(i).rpm  + "      - BPM: " + rings.get(i).bpm  + "          - GSR: " + rings.get(i).gsr);
    //rings.get(i).rpm = correctedData.get(i); //not the same sizes
  }
  println("MinGSR: "+minGSR+"  MaxGSR: "+maxGSR);  
  gotXethruData = true;
